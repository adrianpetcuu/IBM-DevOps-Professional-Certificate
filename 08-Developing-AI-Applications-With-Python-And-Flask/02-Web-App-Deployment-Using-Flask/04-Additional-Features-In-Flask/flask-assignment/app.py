...
def add_transaction():
    ...
    if request.method == 'POST':
        ...
        transaction = {
            'id': len(transactions) + 1,  # Get a new ID based on the current length of the transactions list
...
def search_transactions():
    if request.method == "POST":
        # 1) citește valorile din formular
        try:
            min_amount = float(request.form.get("min_amount", "-inf"))
            max_amount = float(request.form.get("max_amount", "inf"))
        except ValueError:
            # date invalide – întoarce pagina cu un mesaj scurt
            return render_template("search.html", error="Introduceți valori numerice!")

        # 2) filtrează lista în intervalul [min_amount, max_amount]
        filtered_transactions = [
            t for t in transactions
            if min_amount <= float(t["amount"]) <= max_amount
        ]

        # 3) reutilizează șablonul de listare pentru a afișa rezultatul
        #    (afișăm și criteriile ca să știe utilizatorul ce a căutat)
        info = f"Rezultate pentru intervalul [{min_amount}, {max_amount}]"
    ...
    # GET → afișează formularul de căutare
...
def total_balance():
    ...
    # afișează un text simplu
    return f"Total Balance: {balance}"

# Run the Flask app => FOR RUN python app.py in cmd and access http://127.0.0.1:5000
